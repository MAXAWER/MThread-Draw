...
from .gestures import GestureSession
...
def _cmd_record(args) -> int:
    ...
    if args.raw:
        # The kernel events as they arrived: faithful to the last detail, and
        # replayable only on this phone, and only if it allows raw input.
        raw.note = args.note or ""
        raw.save(args.output)
        print(f"Saved {len(raw.events)} raw events ({raw.duration:.1f}s) to {args.output}")
        return 0

    session = GestureSession.from_events(
        raw.events, device.touch_device,
    ...
    if not session.strokes:
        print("Events were captured but none of them were touches.", file=sys.stderr)
        return 1
    session.save(args.output)
    print(f"Saved {len(session.strokes)} strokes, {session.point_count} points "
          f"({session.duration:.1f}s) to {args.output}")
    print("Coordinates are fractions of the screen, so this replays on any phone.")
    return 0
...
def _cmd_play(args) -> int:
    ...
    try:
        session = GestureSession.load(args.input)
    except ValueError as reason:
        # A version 1 file holds digitizer coordinates and replays through
        # /dev/input, which most current phones refuse. Say so, and offer the
        # old path rather than pretending the file is unreadable.
        print(f"error: {reason}", file=sys.stderr)
        print("Replay it as raw events with --raw if this is the phone it was "
              "made on and that phone allows them.", file=sys.stderr)
        if not args.raw:
            return 2
        session = Session.load(args.input)
        print(f"Replaying {len(session.events)} raw events at {args.speed}x")
        replay(device, session, speed=args.speed, repeat=args.repeat)
        print("Done.")
        return 0

    width, height = device.screen_size
    where = f" recorded on {session.device_model}" if session.device_model else ""
    print(f"Replaying {len(session.strokes)} strokes, {session.point_count} points "
          f"({session.duration:.1f}s) at {args.speed}x{where}, scaled to {width}x{height}")

    def progress(done, total):
        print(f"\r  {done}/{total} strokes", end="", flush=True)

    played = device.play_gestures(session, speed=args.speed, repeat=args.repeat,
                                  progress=progress)
    print(f"\nDone, {played} strokes.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    record.add_argument("--raw", action="store_true",
                        help="store kernel events instead: exact, but replayable "
                             "only on this phone and only if it allows raw input")
    ...
    play.add_argument("--raw", action="store_true",
                      help="the file holds kernel events from an older version")
